...
void scheduler()
{
    ...
    while (job_queue->size > 0 || ready_queue->size > 0 || waiting_queue->size > 0) // while there is any process to schedule
    {
        ...
        {
        case SCHEDULING_ALGORITHM_FCFS:
            fcfs_scheduler();
            break;
        case SCHEDULING_ALGORITHM_SJF:
            sjf_scheduler();
            break;
        case SCHEDULING_ALGORITHM_PRIORITY:
            priority_scheduler();
            break;
        }
    }
...
void sjf_scheduler()
{
    ...
    while (job_queue->size > 0)
    {
        ...
        if (p->arrival_time < cpu_counter) // check if process has arrived
        {
            p->flag = PROCESS_FLAG_READY;               // set flag to ready
            dequeue(job_queue, p->pid);                 // remove from job queue
            enqueue_orderby_burst_time(ready_queue, p); // move process from job queue to ready queue based on burst time (shortest job first)
        }
        else
        {
            break; // no more process has arrived yet
    ...
    while (current != NULL) // loop waiting queue to find READY flag processes
    {
        ...
        if (p->flag == PROCESS_FLAG_READY)
        {
            ...
            {
                enqueue_orderby_burst_time(ready_queue, p); // move process from waiting queue to ready queue based on burst time (shortest job first)
            }
        }
    }
    ...
    if (p != NULL) // check if there is any process in ready queue
    {
        ...
        if (p->flag != PROCESS_FLAG_WAITING)
        {
            ...
            {
                p->flag = PROCESS_FLAG_READY;
                enqueue_orderby_burst_time(ready_queue, p); // move back to ready queue (not completed yet) based on burst time (shortest job first)
            }
        }
    }
    else
...
void priority_scheduler()
{
    ...
    while (job_queue->size > 0)
    {
        ...
        if (p->arrival_time < cpu_counter) // check if process has arrived
        {
            p->flag = PROCESS_FLAG_READY;             // set flag to ready
            dequeue(job_queue, p->pid);               // remove from job queue
            enqueue_orderby_priority(ready_queue, p); // move process from job queue to ready queue based on priority
        }
        else
        {
            break; // no more process has arrived yet
    ...
    while (current != NULL) // loop waiting queue to find READY flag processes
    {
        ...
        if (p->flag == PROCESS_FLAG_READY)
        {
            ...
            {
                enqueue_orderby_priority(ready_queue, p); // move process from waiting queue to ready queue based on priority
            }
        }
    }
    ...
    if (p != NULL) // check if there is any process in ready queue
    {
        ...
        if (p->flag != PROCESS_FLAG_WAITING)
        {
            ...
            {
                p->flag = PROCESS_FLAG_READY;
                enqueue_orderby_priority(ready_queue, p); // move back to ready queue (not completed yet) based on priority
            }
        }
    }
    else
